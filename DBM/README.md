...
如果您只想知道如何导入数据而不想全面的了解插件，就不需要继续往下看了，您只需要点击`头像右键`，`菊花插件集`，`打开DBM面板`，`点击右上角的所有数据`选择导入网络数据即可。
...
* 倒计时KEY设置，KEY设置属于高级应用，在现有的倒计时机制无法实现的情况下，可以利用此方法来覆盖现有的任意倒计时（仅需倒计时两个KEY相同，插件即认为这两个倒计时属于同一事件所触发，可以彼此覆盖），可以`按住Ctrl点击倒计时类型下拉框`来打开KEY设置项。
