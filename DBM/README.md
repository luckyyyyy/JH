 ...
 * DBM新手使用说明：http://www.jx3pve.com/thread-9891-1-1.html
...
一个倒计时，由一个数据所驱动，目前插件支持 `BUFF获得` `BUFF消失` `NPC进入场景` `NPC消失` `NPC全部消失` `NPC进入战斗` `NPC死亡` `NPC全部死亡` `技能开始释放` `技能成功释放` `触发喊话` `NPC血量/内力报警（这个有些特殊）`等多种倒计时触发方式。

一个普通的倒计时（血量/内力报警除外）由 `触发方式` `图标` `团队喊话报警` `倒计时时间` `重复调用时间限制` 所组成。
...
* 当NPC血量/内力为72%时 将触发一个中央报警以及特大文字报警，并且触发10秒倒计时。
* 当NPC血量/内力为30%时 将触发一个中央报警以及特大文字报警。

`特别注意`

* 不支持0.724这样的血量格式。
* NPC血量/内力报警暂时不支持调用分段倒计时。
* NPC血量/内力变化跨度瞬间大于50%时，不适用本功能。
* NPC血量只监控递减，即从高到低才会被监控到。
* NPC内力只监控递增，即从低到高才会被监控到。
* 由于游戏本身限制，内力监控若非当前目标或目标的目标，则存在一定的延迟（1-10s）。
