...
-- @Last Modified time: 2015-05-25 14:58:42
...
function PS.OnPanelActive(frame)
	...
	nX, nY = ui:Append("WndCheckBox", { x = 10, y = nY + 5, txt = _L["Enable DBM Plugin"], checked = DBM.bEnable }):Click(function(bCheck)
		DBM_API.Enable(bCheck, true)
		DBM.bEnable = bCheck
	end):Pos_()
	nX, nY = ui:Append("Text", { x = 0, y = nY, txt = _L["Enable alarm (master switch)"], font = 27 }):Pos_()
	nX = ui:Append("WndCheckBox", { x = 10, y = nY + 5, txt = _L["Team Channel Alarm"], color = GetMsgFontColor("MSG_TEAM", true), checked = DBM.bPushTeamChannel }):Click(function(bCheck)
		DBM.bPushTeamChannel = bCheck
	end):Pos_()
	nX = ui:Append("WndCheckBox", { x = nX + 5, y = nY + 5, txt = _L["Whisper Channel Alarm"], color = GetMsgFontColor("MSG_WHISPER", true), checked = DBM.bPushWhisperChannel }):Click(function(bCheck)
		DBM.bPushWhisperChannel = bCheck
	end):Pos_()
	nX = ui:Append("WndCheckBox", { x = nX + 5, y = nY + 5, txt = _L["Buff List"], checked = DBM.bPushBuffList }):Click(function(bCheck)
		DBM.bPushBuffList = bCheck
	end):Pos_()
	nX, nY = ui:Append("WndCheckBox", { x = nX + 5, y = nY + 5, txt = _L["Center Alarm"], checked = DBM.bPushCenterAlarm }):Click(function(bCheck)
	...
	nX, nY = ui:Append("WndCheckBox", { x = 10, y = nY + 5, txt = _L["Team Panel Bind Show Buff"], checked = DBM.bPushTeamPanel }):Click(function(bCheck)
	...
	nX, nY = ui:Append("WndCheckBox", { x = 10, y = nY + 5, txt = _L["Use common data"], checked = DBM.bCommon }):Click(function(bCheck)
		DBM.bCommon = bCheck
	end):Pos_()
